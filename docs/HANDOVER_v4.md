...
- ★ **모바일 UX 개선** — 세로/가로 모드 둘 다 이슈 있음. 한림 iPhone에서 확인(2026-06-01).
  - 세로(portrait): 판이 작아 기물 글자 가독성 저하. 9×10 보드를 세로 폰에 넣는 구조적 한계.
  - 가로(landscape): 판 아래 줄(초 뒷줄) 잘림 + 사이드 패널이 좁아져 "언어 선택 :" 글자 세로 쪼개짐, 상태창 문구 3줄로 줄바꿈.
  - 수정 방향: `@media (orientation: landscape) and (max-width: 1024px)` 전용 규칙 추가 — 판 폭 축소(flex 비율 조정), 사이드 패널 최소 너비 확보, 판 높이가 뷰포트 안에 들어오게. 세로는 판 최대화 + UI 접기 or 가로 모드 유도 안내.
  - ★ **레퍼런스: Chess.com 모바일** — 판을 화면 꽉 채우고, 컨트롤(무르기·기권·힌트 등)을 하단 탭바로 내림. 상단은 최소화. 덕분에 세로 폰에서도 판이 크게 잡힘. 우리도 사이드 패널 대신 **"판 최대화 + 하단 탭바(무르기·처음부터·돌 던지기)"** 구조로 모바일 전용 레이아웃 고려할 것. 기보·잡은 기물은 접어두고 필요할 때 꺼내는 방식. ※ Chess.com의 AI 캐릭터(Captain Rook 등) 방향은 우리 수묵화 톤과 달라 적용 안 함 — 구조만 참고.
  - 우선순위: AI 대국 다음. 급하진 않으나 모바일 접속자 경험에 직결.
...
- ★ **멀티플레이 (온라인 대전)** — GitHub Pages만으론 불가, **별도 서버 필요**. 두 가지 방식:
  - WebSocket 서버(Node.js + Socket.io): 수 데이터를 서버가 중개. 구현 단순하나 서버 상시 운영 필요(Render·Railway·Fly.io 무료 플랜 가능하나 슬립 제약).
  - WebRTC P2P: 시그널링 서버만 두고 브라우저끼리 직접 연결. 서버 부하 낮아 무료 운영에 유리하나 구현 복잡.
  - ★ **현재 클라이언트 구조는 이미 준비됨**: `playerFaction`="화면의 주인", `flipped`로 진영만 뒤집는 구조가 네트워크 대비로 설계됨(인수인계서 3번 참조). 서버 레이어만 새로 붙이면 됨.
  - 우선순위: AI 대국 → 빅장/룰 옵션 → 멀티플레이 순 권장. 지금은 아이디어 뱅크 보류.
