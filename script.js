(function () {
  ...
  //
  // ═══════════════════════════════════════════════════════════════════════
  // ★★ 난이도 조절 가이드 (한림 혼자 수정용) — 아래 표의 숫자만 바꾸면 됨
  // ═══════════════════════════════════════════════════════════════════════
  //  손잡이 4개. 각각 무슨 역할이고 어느 쪽으로 돌리면 어떻게 되는지:
  //
  //  ┌ blunder (0 ~ 1) ── ★ 입문 난이도의 핵심 손잡이. 제일 먼저 여길 만져라.
  //  │   "이 수에 엔진을 안 부르고 무작위로 둘 확률". 높을수록 AI가 자주 헛발질 → 쉬움.
  //  │   0    = 실수 안 함(순수 엔진)
  //  │   0.2  = 다섯 수에 한 번꼴 무작위 (현재 beginner — "가끔 이기고 가끔 진다")
  //  │   0.5+ = 절반 이상 무작위 (너무 쉬움·일방적. 측정상 "AI가 바보 같다")
  //  │   ※ 0.05씩 움직여라. 0.1 단위로 크게 바꾸면 체감이 확 튐(측정 때 0.3→0.2가 압승→패배였음).
  //  │
  //  ├ depth (1 ~ 15+) ── AI가 몇 수 앞을 내다보는지. 평소(블런더 안 터질 때) 실력.
  //  │   낮으면 코앞만 봄(약함), 높으면 길게 계산(강함·느려짐).
  //  │   blunder로 "가끔 실수"를 만들고, depth로 "평소 실력"을 받친다. 둘은 다른 축.
  //  │   ※ depth 올리면 생각이 길어져 thinkMax(아래)에 부딪힐 수 있음 — 너무 높이면 답답.
  //  │
  //  ├ skill (0 ~ 20) ── Fairy-Stockfish 자체 강도. 0이라도 Elo 1320이라 입문자껜 여전히 셈.
  //  │   → 그래서 입문 난이도는 skill이 아니라 blunder로 만든다(skill만으론 약화 한계).
  //  │   중급 이상 미세조정용. 평소엔 안 건드려도 됨.
  //  │
  //  └ thinkMin / thinkMax (ms) ── "생각하는 척" 시간. 강도와 무관, 인격(호흡)만 담당.
  //      계산이 빨리 끝나도 이 시간 동안 "생각 중" 유지(2500 = 2.5초).
  //      ※ 약하다고 빨리 두게 하지 마라 — 빠르면 "약함"이 아니라 "계산기 느낌"이 남.
  //      ※ 강도엔 영향 0. 답답하면 줄이고, 너무 빨라 기계 같으면 늘린다.
  //
  //  ── 흔한 상황별 처방 (한 번에 하나씩만 바꾸고 몇 판 둬본 뒤 판단) ──
  //   "초보자가 너무 쉽다 (맨날 이김)"     → beginner blunder ↓ (0.2 → 0.15)
  //   "초보자가 너무 어렵다 (맨날 짐)"     → beginner blunder ↑ (0.2 → 0.25 ~ 0.3)
  //   "쉬운데 AI가 바보처럼 막 던진다"    → blunder ↓ + depth ↑ (평소 실력을 올려 '예리한 순간'을 만듦)
  //   "한 단계씩 차이가 잘 안 느껴진다"   → 단계 간 blunder 간격을 벌린다(예: 0.2 / 0.1 / 0.05)
  //   "AI가 너무 느려 답답하다"          → thinkMax ↓ (단 2500 밑으론 내리지 마라, 기계 느낌)
  //   "공짜 기물 너무 쉽게 흘린다"        → 맨 위 BLUNDER_MODE 를 'mercy'로 (잡는 수는 일부러 안 흘림)
  //
  //  ★ 측정 원칙(measure not guess): 화면 체감은 한림이 먼저. 숫자 하나 바꿀 때마다
  //    몇 판 둬보고 "쉬워졌나/어려워졌나"를 몸으로 확인한 뒤 다음 조정. 두 개 동시에 바꾸면
  //    뭐가 효과였는지 모름. beginner는 한림 자신이 초심자라 가장 정확한 테스트 대상.
  // ═══════════════════════════════════════════════════════════════════════
  const AI_LEVELS = {
  ...
  const TUTORIAL_I18N = {
    ko: {
      ...
      // 기물 성격 한 줄 (intro에만 표시 — desc가 '어떻게'라면 trait는 '어떤 기물인가'. 사실·일반론, 전략 처방 X)
      tutRookTrait: '가장 멀리 뻗어 나가는 기물 — 길이 열릴수록 강합니다.',
      tutHorseTrait: '길목이 막히면 발이 묶이는, 자리를 가리는 기물입니다.',
      tutElephantTrait: '마보다 멀리 내딛지만, 그만큼 길이 막히기도 쉽습니다.',
      tutCannonTrait: '홀로는 움직이지 못하고, 받침이 있어야 힘을 내는 기물입니다.',
      tutGuardTrait: '궁성을 떠나지 않는, 장을 지키는 수비의 기물입니다.',
      tutKingTrait: '한 칸씩 움직이지만, 잃으면 모든 것이 끝나는 기물입니다.',
      tutSoldierTrait: '앞으로만 나아가는, 물러설 줄 모르는 기물입니다.',
    ...
    en: {
      ...
      tutRookTrait: 'The piece that reaches farthest — stronger the more the lines open up.',
      tutHorseTrait: 'A piece that picks its ground; block its path and it is stuck.',
      tutElephantTrait: 'Reaches farther than the Horse, but is just as easily blocked.',
      tutCannonTrait: 'Cannot move alone — it needs a screen to show its strength.',
      tutGuardTrait: 'A defender that never leaves the palace, guarding the General.',
      tutKingTrait: 'Moves one step at a time, yet losing it ends everything.',
      tutSoldierTrait: 'Ever forward, never back — a piece that does not retreat.',
    ...
    'zh-Hans': {
      ...
      tutRookTrait: '走得最远的棋子——路越开阔，威力越强。',
      tutHorseTrait: '挑剔落脚之处的棋子，路一被堵就动弹不得。',
      tutElephantTrait: '比马走得更远，却也同样容易被挡住。',
      tutCannonTrait: '无法独自行动，须有炮架方能发挥力量。',
      tutGuardTrait: '从不离开宫城、守护将帅的防守棋子。',
      tutKingTrait: '每次只走一格，可一旦失去便满盘皆输。',
      tutSoldierTrait: '一味向前、绝不后退的棋子。',
    ...
    'zh-Hant': {
      ...
      tutRookTrait: '走得最遠的棋子——路越開闊，威力越強。',
      tutHorseTrait: '挑剔落腳之處的棋子，路一被堵就動彈不得。',
      tutElephantTrait: '比馬走得更遠，卻也同樣容易被擋住。',
      tutCannonTrait: '無法獨自行動，須有炮架方能發揮力量。',
      tutGuardTrait: '從不離開宮城、守護將帥的防守棋子。',
      tutKingTrait: '每次只走一格，可一旦失去便滿盤皆輸。',
      tutSoldierTrait: '一味向前、絕不後退的棋子。',
    ...
    ja: {
      ...
      tutRookTrait: '最も遠くまで伸びる駒——道が開けるほど強くなります。',
      tutHorseTrait: '進む先を選ぶ駒。道をふさがれると動けなくなります。',
      tutElephantTrait: '馬より遠くへ進めますが、その分ふさがれやすい駒です。',
      tutCannonTrait: '単独では動けず、台があってこそ力を発揮する駒です。',
      tutGuardTrait: '宮城を離れず、将を守る守りの駒です。',
      tutKingTrait: '一マスずつしか動けませんが、失えばすべてが終わる駒です。',
      tutSoldierTrait: '前へ進むのみ、退くことを知らない駒です。',
    ...
    de: {
      ...
      tutRookTrait: 'Die Figur mit der größten Reichweite — stärker, je offener die Linien.',
      tutHorseTrait: 'Eine wählerische Figur; ist ihr Weg versperrt, steht sie fest.',
      tutElephantTrait: 'Reicht weiter als das Pferd, lässt sich aber ebenso leicht blockieren.',
      tutCannonTrait: 'Zieht nie allein — sie braucht einen Bock, um ihre Kraft zu zeigen.',
      tutGuardTrait: 'Ein Verteidiger, der den Palast nie verlässt und den General schützt.',
      tutKingTrait: 'Zieht nur Schritt für Schritt, doch sein Verlust beendet alles.',
      tutSoldierTrait: 'Immer vorwärts, niemals zurück — eine Figur, die nicht weicht.',
    ...
    fr: {
      ...
      tutRookTrait: 'La pièce à la plus longue portée — plus les lignes s’ouvrent, plus elle est forte.',
      tutHorseTrait: 'Une pièce qui choisit son terrain ; barrez sa route et elle est bloquée.',
      tutElephantTrait: 'Va plus loin que le Cheval, mais se laisse bloquer tout aussi aisément.',
      tutCannonTrait: 'Ne se déplace jamais seul — il lui faut un appui pour montrer sa force.',
      tutGuardTrait: 'Un défenseur qui ne quitte jamais le palais, gardien du Général.',
      tutKingTrait: 'Avance pas à pas, mais le perdre, c’est tout perdre.',
      tutSoldierTrait: 'Toujours en avant, jamais en arrière — une pièce qui ne recule pas.',
  ...
  function renderTutorialSidePanel(scenarioId) {
    ...
    if (!scenario) {
      ...
      html =
        ...
        (scenario.traitKey ? `<div class="tut-trait">${tt(scenario.traitKey)}</div>` : '') +
  ...
  const MODE_LIST = [
    ...
    { id: 'tutorial', nameKey: 'modeTutorial', subKey: 'modeTutorialSub', active: true  },
